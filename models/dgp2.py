...
class DGP(object):
    ...
    def run(self, mask, save_interval=None):
        ...
        for stage, iteration in enumerate(self.iterations):
            ...
            for i in range(iteration):
                ...
                x_map = self.pre_process(x, mask,False)
                ...
                if self.mode in ['reconstruct', 'colorization', 'SR', 'inpainting']:
                    ...
                    metrics, x2 = self.get_metrics(x, mask)
    ...
    def select_z(self, mask, select_y=False):
        with torch.no_grad():
    ...
    def pre_process(self, image, mask,target=True):
