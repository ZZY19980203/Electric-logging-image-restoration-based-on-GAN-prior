...
img, mask = utils.get_img(config['image_path'],config['mask_path'], config['resolution'])
img = img.cuda()
mask = mask.cuda()
print(mask.max())
category = torch.Tensor([config['class']]).long().cuda()
dgp.set_target(img, mask, category, config['image_path'])

# prepare initial latent vector
dgp.select_z(mask, select_y=True if config['class'] < 0 else False)
# start reconstruction
loss_dict = dgp.run(mask)
